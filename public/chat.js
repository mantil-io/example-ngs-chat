...
import { connect, JSONCodec, StringCodec, credsAuthenticator, createInbox } from "./nats.js";
const me = Date.now();
...
const ngsConnect = async function() {
  ...
  // fetch the creds
  const creds = await fetch('./tester.creds')
  if (!creds.ok) {
    addEntry("unable to find ./tester.creds - aborting")
    return;
  }
...
const init = async function () {
  ...
  const sub = conn.subscribe(inbox);
  (async () => {
...
function send() {
  ...
  if (m !== "" && window.nc) {
    const msg = { id: me, m: m };
    //window.nc.publish("chat", jc.encode(msg));
    post(msg);
    input.value = "";
  }
  return false;
